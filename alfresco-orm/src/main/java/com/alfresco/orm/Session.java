...
public class Session
{
	...
	public void save(final AlfrescoORM alfrescoORM) throws ORMException
	{
		...
		try
		{
			...
			NodeRef nodeRef = createContent(alfrescoContent, properties, alfrescoORM);

			if (null != nodeRef)
			{
				AlfrescoVO alfrescoVO = (AlfrescoVO) alfrescoORM;
				alfrescoVO.setNodeUUID(nodeRef.toString());
				saveProperties(alfrescoORM, properties, nodeRef);
			}
	 ...
	 * @throws ORMException
	 */
	private void saveProperties(final AlfrescoORM alfrescoORM, final Map<QName, Serializable> properties, final NodeRef nodeRef)
			throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, ORMException
	{
		getServiceRegistry().getNodeService().setProperties(nodeRef, properties);
	 ...
	 * @throws ORMException
	 */
	private Object getTargetServiceBean(final SpringBeanID springBeanID) throws ORMException
	{
	 ...
	 * @throws InvocationTargetException
	 */
	public NodeRef getParentContent(final AlfrescoType alfrescoType, final AlfrescoORM alfrescoORM) throws ORMException, SecurityException,
	 ...
	 * @throws InvocationTargetException
	 */
	public NodeRef createNodeMethodName(final AlfrescoType alfrescoType, final AlfrescoORM alfrescoORM) throws ORMException, SecurityException,
			NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException
	{
		Object target = getTargetServiceBean(alfrescoType.springBeanID());
	 ...
	 * @throws InvocationTargetException
	 */
	private NodeRef createContent(final AlfrescoType alfrescoContent, final Map<QName, Serializable> properties, final AlfrescoORM alfrescoORM)
			throws ORMException, NoSuchMethodException, IllegalAccessException, InvocationTargetException
	{
		...
		{
			NodeRef parentNodeRef = getParentContent(alfrescoContent, alfrescoORM);
			if (null != parentNodeRef && getServiceRegistry().getNodeService().exists(parentNodeRef))
			{
				QName fileQName = QName.createQName(alfrescoContent.alfrescoQName().namespaceURI(), alfrescoContent.alfrescoQName().localName());
				FileInfo fileInfo = getServiceRegistry().getFileFolderService().create(parentNodeRef,
						(String) properties.get(ContentModel.PROP_NAME), fileQName);
	...
	public void executeCustomeMethodForProperty(final AlfrescoORM alfrescoORM, final NodeRef nodeRef) throws SecurityException,
			NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException, ORMException
	{
		...
		for (Field field : fields)
		{
			if (field.isAnnotationPresent(CustomProperty.class))
			{
				...
				{
					Object target = getTargetServiceBean(customProperty.springBeanID());
	...
	public Map<QName, Serializable> getAlfrescoProperty(final AlfrescoORM alfrescoORM) throws IllegalArgumentException, IllegalAccessException,
			SecurityException, NoSuchMethodException, InvocationTargetException
	{
		...
		for (Field field : fields)
		{
			if (field.isAnnotationPresent(Property.class))
			{
				...
				QName qName = QName.createQName(property.alfrescoQName().namespaceURI(), property.alfrescoQName().localName());
